...
echo '<p><form name=subjectsForm class=iactive method=post><table cellpadding=0 cellspacing=0 border=0 width="100%"><tr><td>';
echo '<table cellpadding=0 cellspacing=0 border=0 width="100%" style="background: url(images/toolbar-background.gif) repeat-x"><tr vAlign=center><td width=2><img src="images/toolbar-left.gif" width=2 height=32></td><td width=32><a href="subjects.php?action=create"><img src="images/button-new-big.gif" border=0 title="'.$lngstr['label_action_create_subject'].'"></a></td><td width=3><img src="images/toolbar-separator.gif" width=3 height=32 border=0></td><td width=32><img src="images/button-cross-big.gif" border=0 title="'.$lngstr['label_action_subjects_delete'].'" style="cursor: hand;" onclick="f=document.subjectsForm;if (confirm(\''.$lngstr['qst_delete_subjects'].'\')) { f.action=\'subjects.php?action=delete&confirmed=1\';f.submit();}"></td><td width="100%">&nbsp;</td>';
if($i_limitcount > 0) {
...
for($i = $nStartPage; $i <= $nEndPage; $i++) {
 if($i != $i_pageno)
 echo '&nbsp;<a href="subjects.php?pageno='.$i.$i_url_pages_addon.'">'.$i.'</a>&nbsp;';
else echo '<span class=currentitem>&nbsp;'.$i.'&nbsp;</span> ';
}
echo '</nobr></td>';
echo '<td width=3><img src="images/toolbar-separator.gif" width=3 height=32 border=0></td>';
if($i_pageno > 1) {
 echo '<td width=32><a href="subjects.php?pageno=1'.$i_url_pages_addon.'"><img src="images/button-first-big.gif" border=0 title="'.$lngstr['button_first_page'].'"></a></td>';
echo '<td width=32><a href="subjects.php?pageno='.max(($i_pageno-1), 1).$i_url_pages_addon.'"><img src="images/button-prev-big.gif" border=0 title="'.$lngstr['button_prev_page'].'"></a></td>';
} else {
...
if($i_pageno < $i_pageno_count) {
 echo '<td width=32><a href="subjects.php?pageno='.min(($i_pageno+1), $i_pageno_count).$i_url_pages_addon.'"><img src="images/button-next-big.gif" border=0 title="'.$lngstr['button_next_page'].'"></a></td>';
echo '<td width=32><a href="subjects.php?pageno='.$i_pageno_count.$i_url_pages_addon.'"><img src="images/button-last-big.gif" border=0 title="'.$lngstr['button_last_page'].'"></a></td>';
} else {
...
writeQryTableHeaders('subjects.php?action='.$i_url_limit_addon, $i_tablefields, $i_order_no, $i_direction);
...
echo '<tr id=tr_'.$i_pagewide_id.' class='.$rowname.' onmouseover="rollTR('.$i_pagewide_id.',1);" onmouseout="rollTR('.$i_pagewide_id.',0);"><td align=center width=22'.($i_rSet1->fields["subjectid"] > SYSTEM_SUBJECTS_MAX_INDEX ? '' : ' class=system').'><input id=cb_'.$i_pagewide_id.' type=checkbox name=box_subjects[] value="'.$i_rSet1->fields["subjectid"].'" onclick="toggleCB(this);"></td><td align=right>'.$i_rSet1->fields["subjectid"].'</td><td>'.getTruncatedHTML($i_rSet1->fields["subject_name"]).'</td><td>'.$i_rSet1->fields["subject_description"].'</td>';
echo '<td align=center width=22><a href="subjects.php?subjectid='.$i_rSet1->fields["subjectid"].'&action=edit"><img width=20 height=20 border=0 src="images/button-edit.gif" title="'.$lngstr['label_action_subject_edit'].'"></a></td><td align=center width=22>'.($i_rSet1->fields["subjectid"] > SYSTEM_SUBJECTS_MAX_INDEX ? '<a href="subjects.php?subjectid='.$i_rSet1->fields["subjectid"].$i_url_limit_addon.'&action=delete" onclick="return confirmMessage(this, \''.$lngstr['qst_delete_subject'].'\')"><img width=20 height=20 border=0 src="images/button-cross.gif" title="'.$lngstr['label_action_subject_delete'].'"></a>' : '<img width=20 height=20 border=0 src="images/button-cross-inactive.gif">').'</td></tr>';
$i_counter++;
$i_pagewide_id++;
$i_rSet1->MoveNext();
}
